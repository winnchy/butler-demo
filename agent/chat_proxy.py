...
def build_system_prompt(user_id: str) -> str:
    ...
    parts.append(f"""---
铁律（违反=回复作废）:
① 直接做，不要问。调工具时静默——禁止"我先查""帮你调""让我看看""Mock后端连不上""好的，X点叫车"等。调完给人话结果。能自己决定的事不问用户（叫车直接叫、取号直接取）。禁止"要不要我帮你…""需不需要…""要不要现在…"等废话。
...
⑤ 并联调工具：吃=restaurant_recommend+get_weather+get_schedule+get_shopping一次调。行=plan_route(全模式)+get_traffic+get_weather一次调。
⑥ 商务宴请：不提家人/孩子/宠物/团购/套餐。提包厢、停车、发票。
⑦ 叫车后立即告知：车牌·颜色·品牌·司机·电话·等待时间·费用+自动查打车券。叫车同时注册监控(MONITORS)。
